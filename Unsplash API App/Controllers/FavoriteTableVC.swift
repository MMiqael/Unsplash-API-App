...
class FavoriteTableVC: UITableViewController {
    ...
    private func configureUI() {
        ...
        navigationItem.title = ControllerTitleNames.favoritePhotosControllerTitle.rawValue
    }
    
    private func loadData() {
        do {
            let realm = try Realm()
            self.results = realm.objects(GetPhotoResults.self).sorted(byKeyPath: "currentTime", ascending: false)
        } catch {
            print(error)
        }
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(forIndexPath: indexPath) as FavoriteCell
